...
describe('MetricsInterceptor', () => {
  ...
  it('uses req.url when routerPath missing and defaults status 200', (done) => {
    ...
    const ctx = {
      getType: () => 'http',
      switchToHttp: () => ({
        getRequest: () => ({ method: 'post', url: '/v1/fallback' }),
        getResponse: () => ({}),
      }),
    ...
    interceptor.intercept(ctx, next).subscribe(() => {
      expect(startTimerMock).toHaveBeenCalledWith({ method: 'POST', route: '/v1/fallback' });
      expect(endMock).toHaveBeenCalledWith({ status: '200' });
      done();
    });
  });
