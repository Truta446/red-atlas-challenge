...
describe('AnalyticsController', () => {
  ...
  it('monthly-percentiles defaults months to 12 on invalid input', async () => {
    const user = { tenantId: 't2' } as any;
    await controller.monthly(user, '-5');
    expect(service.monthlyPercentiles).toHaveBeenCalledWith('t2', 12);
    await controller.monthly(user, 'abc');
    expect(service.monthlyPercentiles).toHaveBeenCalledWith('t2', 12);
  });
  ...
  it('monthly-percentiles defaults to 12 on invalid months', async () => {
    const user = { tenantId: 't2' } as any;
    await controller.monthly(user, '-1');
    expect(service.monthlyPercentiles).toHaveBeenCalledWith('t2', 12);
    await controller.monthly(user, 'abc');
    expect(service.monthlyPercentiles).toHaveBeenCalledWith('t2', 12);
  });
  ...
  it('sector-yoy uses undefined when year missing or invalid', async () => {
    const user = { tenantId: 't3' } as any;
    await controller.yoy(user, undefined as any);
    expect(service.sectorYoYValuation).toHaveBeenCalledWith('t3', undefined);
    await controller.yoy(user, 'NaN');
    expect(service.sectorYoYValuation).toHaveBeenCalledWith('t3', undefined);
  });
});
